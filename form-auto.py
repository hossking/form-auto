...
# 发送邮件通知
# receivers = ['xx']  # 接收邮箱,可设置为你的QQ邮箱或者其他邮箱
...
def submit():
    ...
    while True:
        ...
        # TODO
        # 验证码 配合OCR识别
        # url = 'http://authserver.sziit.edu.cn/authserver/needCaptcha.html'
        # params1 = {
        #     'username': xh,
        ...
        # 携带cookie查询最新待提交表单
        queryCollectWidUrl = 'https://sziit.cpdaily.com/wec-counselor-collector-apps/stu/collector/queryCollectorProcessingList'
        headers = {
        ...
        res = requests.post(url='https://sziit.cpdaily.com/wec-counselor-collector-apps/stu/collector/detailCollector',
        ...
        for i in range(len(form) - 1, -1, -1):
            if form[i]['fieldItems']:
                ...
                for item in Items[:]:
                    # 有新增必选项可在列表中补充
                    if item['content'] in ['否', '未去过', '不在', '都未去过', '都不在湖北']:
                        continue
                    else:
                        Items.remove(item)
        ...
        headers = {
            ...
            'Content-Type': 'application/json; charset=utf-8',
            'Host': 'sziit.cpdaily.com',
            'Connection': 'Keep-Alive',
            'Accept-Encoding': 'gzip'
        }
        # 地址根据学校要求填写即可
        params = {"formWid": formWid, "address": "",
                  "collectWid": collectWid, "schoolTaskWid": schoolTaskWid,
                  "form": form
        }

        r = server.post("http://sziit.cpdaily.com/wec-counselor-collector-apps/stu/collector/submitForm",
